...
export default function SpotifyWidget() {
    ...
    if (user) {
      try {
        const { data } = await supabase
          ...
          .maybeSingle();
        if (data?.items?.[0]) {
          const info = data.items[0] as any;
          if (info.playlistId) {
            setPlaylistId(info.playlistId);
            setPlaylistName(info.playlistName || "");
            setPlaylistUrl(info.spotifyUrl || "");
  ...
  async function savePlaylistId(id: string, name: string, url: string) {
    const info = { playlistId: id, playlistName: name, spotifyUrl: url };
    try {
      localStorage.setItem("spotify_playlist", JSON.stringify(info));
    } catch {}
    if (!user) return;
    try {
      ...
      console.error("Save playlist to Supabase error:", e);
    }
  }
  ...
  return (
    <div className="rounded-2xl border border-border bg-card overflow-hidden">
      <div className="px-4 py-3 flex items-center justify-between border-b border-border/30">
        ...
        {playlistUrl && (
          <a href={playlistUrl} target="_blank" rel="noopener noreferrer" className="text-[14px] text-muted-foreground hover:text-[#1DB954] flex items-center gap-1">
      ...
      {nowPlaying?.playing && nowPlaying.track && (
        <div className="px-4 py-3 bg-[#1DB954]/5 border-b border-border/30">
          <p className="text-[14px] uppercase tracking-wider font-semibold text-[#1DB954] mb-2 flex items-center gap-1.5">
