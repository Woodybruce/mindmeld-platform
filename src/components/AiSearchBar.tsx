  ...
  if (!open) {
    return (
      ...
      >
        ...
        <span className="text-[13px] text-muted-foreground">Ask AI to find the best {section === "discover" ? "products & experiences" : "articles, podcasts & videos"}...</span>
      </button>
    );
  }
        ...
        <div className="flex gap-1.5 mt-2 overflow-x-auto scrollbar-hide">
          {QUICK_PROMPTS[section].map((prompt) => (
            <button
              ...
              className="flex-shrink-0 text-[14px] px-2.5 py-1 rounded-full bg-primary/5 text-primary hover:bg-primary/10 transition-colors whitespace-nowrap"
      ...
      <AnimatePresence>
        {loading && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex items-center gap-2 mt-3 px-1">
            ...
            <span className="text-[14px] text-muted-foreground">Finding the best recommendations...</span>
          </motion.div>
        )}

        {searched && !loading && results.length === 0 && (
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-[14px] text-muted-foreground mt-3 px-1">
            ...
            {results.map((item, i) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[14px] text-muted-foreground mt-0.5 line-clamp-2">{item.description}</p>
                  <div className="flex items-center gap-2 mt-1.5">
                    {item.price && <span className="text-[13px] font-bold text-primary">{item.price}</span>}
                    {item.brand && <span className="text-[13px] text-muted-foreground">{item.brand}</span>}
                    {item.venue && <span className="text-[13px] text-muted-foreground">{item.venue}</span>}
                    {item.host && <span className="text-[13px] text-muted-foreground">{item.host}</span>}
                    {item.creator && <span className="text-[13px] text-muted-foreground">{item.creator}</span>}
                    {item.duration && <span className="text-[13px] text-muted-foreground">· {item.duration}</span>}
                    {item.destination && <span className="text-[13px] text-muted-foreground">{item.destination}, {item.country}</span>}
                    {item.source && <span className="text-[13px] text-muted-foreground">{item.source}</span>}
                    {item.author && <span className="text-[13px] text-muted-foreground italic">— {item.author}</span>}
                  </div>
                  {item.type === "quote" && item.text && (
                    <p className="text-[14px] italic text-foreground/70 mt-1 line-clamp-3">"{item.text}"</p>
                  )}
                  {getActionLabel(item.type) && (
                    <button
                      onClick={() => openExternal(item)}
                      className={`mt-2 inline-flex items-center gap-1 text-[14px] font-semibold px-2.5 py-1 rounded-lg transition-colors ${
                        item.type === "product"
